...
-- Create function to update updated_at timestamp for farm_performance
CREATE OR REPLACE FUNCTION public.update_farm_performance_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Create function to update updated_at timestamp for dashboard_metrics
CREATE OR REPLACE FUNCTION public.update_dashboard_metrics_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER update_farm_performance_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.update_farm_performance_updated_at();

CREATE TRIGGER update_dashboard_metrics_updated_at
  BEFORE UPDATE ON public.dashboard_metrics
  FOR EACH ROW
  EXECUTE FUNCTION public.update_dashboard_metrics_updated_at();
